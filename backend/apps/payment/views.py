...
def payment_status(request, payment_id):
    try:
        ...
        if payment.stripe_session_id and payment.status in ['pendindg','processing']:
        ...
        if payment.is_succesful and payment.subscription:
...
def create_refund(request,payment_id):
    try:
        ...
        serializer=RefundCreateSerialiezr(
            data=request.data,
        ...
        if serializer.is_valid():
            with transaction.atomic():
                ...
                if success:
                    ...
                    if refund.amount == payment.amount and payment.subsctiption:
...
def payment_analytics(request):
    ...
    succesful_payments = Payment.objects.filter(status='succeeded').count()
    total_revenue = Payment.objects.filter(
        status='succeeded'
    ).aaggregate(total=Sum('amount'))['tota'] or 0
    ...
    monthly_revenue = monthly_payments.aaggregate(
        total=Sum('amount')
    )['total'] or 0
    ...
    return Response({
        ...
        'succesful_payments': succesful_payments,
        'success_rate': (succesful_payments / total_payments * 100) if total_payments > 0 else 0,
