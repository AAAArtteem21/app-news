...
class SubscriptionPlanSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            'id','name','price','duration_days','features','is_active','created_at'
        ]
...
class  SubscriptionCreateSerializer(serializers.ModelSerializer):
    ...
    def validate(self,attrs):
        ...
        if hasattr(user,'subscription') and user.subscriptions.is_active():
            raise serializers.ValidationError({
                'non_field_error': ['User already has an active subscription.']
            })
...
class PinnedPostSerializer(serializers.ModelSerializer):
    ...
    def validate_post(self,value):
        ...
        if value.status != 'published':
            raise serializers.ValidationError('Only published post can be pinned.')
...
class PinPostSerializer(serializers.Serializer):
    ...
    def validate_post_id(self,value):
        ...
        user = self.context['request'].user
        if post.author != user:
            raise serializers.ValidationError("You can only pin youu own posts.")
    ...
    def validate(self, attrs):
        ...
        if not hasattr(user,'subscription') or not user.subscription.is_active:
            raise serializers.ValidationError({
                'non_field_error':['Active subscription requried to pin posts.']
            })   
